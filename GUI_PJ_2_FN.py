...
if choice == 'Tổng Quan':
    ...
    with gzip.GzipFile('cosine_similarity_FA.npy.gz', 'rb') as f:
            ...
            if not matching_products.empty:
                ...
                for index, row in result_df.iloc[1:].iterrows():
                    with product_container.expander(f"Mã sản phẩm: {row['ma_san_pham']} - {row['ten_san_pham']}"):
                        ...
                        st.write(f"**Giá bán:** {int(row['gia_ban']):,} VNĐ")
    ...
    if id:
        ...
        try:
            ...
            st.write(f"**✴️Mã khác hàng:** {id}")
             
            sp = sp = pd.read_csv('All_San_pham_clean.csv',sep=';')

            # Đề xuất mua hàng:
            result_id= recommend_userid(id,algorithm,df,6)
            ...
            for i in range(0, len(x), 3):
                ...
                for j, col in enumerate(cols):
                    if i + j < len(x):  # Kiểm tra có đủ sản phẩm không
                        product = x.iloc[i + j]
                        col.write(f"**{product['ten_san_pham']}**")
                        col.write(f"Mã sản phẩm: {product['ma_san_pham']}")
                        col.write(f"Điểm: {product['diem_trung_binh']}")
                        col.write(f"Giá: {int(product['gia_ban']):,} VNĐ")
                        a = sp.loc[sp['ma_san_pham'] == product['ma_san_pham'], 'mo_ta'].iloc[0]
                        with col:
